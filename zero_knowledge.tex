...
\title{\textbf{Zero Knowledge: An Introduction}}
\author{A collaborative learning note}
\date{}
...
%% ============================================================
\section{Exercise: The Apple Test}
%% ============================================================

\begin{tcolorbox}[colback=orange!5!white,colframe=orange!60!black,boxrule=0.6pt,arc=2mm]
  \textbf{Exercise.}
...
\begin{tcolorbox}[colback=orange!5!white,colframe=orange!60!black,boxrule=0.6pt,arc=2mm]
\textbf{Exercise.} Formally prove all three properties for the Schnorr protocol. For honest-verifier ZK, write down the explicit simulator and show the simulated and real distributions over $(a, c, z)$ are identical.
\end{tcolorbox}
...
A proof system has \emph{perfect zero knowledge} if the simulated transcript is \emph{identically distributed} to the real transcript (not merely computationally indistinguishable).
\end{definition}

\textbf{Why they cannot coexist.} Recall how the Schnorr simulator works: to fake a transcript for challenge $c$, it picks $z \leftarrow \Zq$ uniformly and sets $a \deq g^z \cdot h^{-c}$. The transcript $(a,c,z)$ is accepted and perfectly distributed. The simulator achieves this by working \emph{backwards} --- it sees $c$ before choosing $a$.

A real cheating prover cannot do this: they must commit to $a$ before the verifier sends $c$. That ordering is exactly what soundness relies on.

If perfect soundness held, then for any false statement $h \notin \lang$, \emph{no} transcript $(a, c, z)$ should ever pass the verifier --- the probability must be exactly zero for every $c$. Yet perfect ZK demands that simulated transcripts (produced without the witness) are identically distributed to real ones. For a nontrivial language these two requirements clash: the simulator's freedom to pick $z$ first and work backwards produces accepting transcripts that perfect soundness forbids. The incompatibility is unavoidable unless the language is trivial.

In practice, proof systems therefore settle for \emph{computational} distinguishability and a \emph{negligible} soundness error --- exactly what Schnorr achieves with error $1/q$.
