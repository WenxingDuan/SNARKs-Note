...
\label{sec:polynomial-commitments}
...
\begin{intuitionbox}[title=\textbf{Intuition for $r$}]
  The folded claimed value $v' = rv_L + v_R$ can be seen as a one-time MAC, so it perfectly hides the original values $v_L$ and $v_R$. 
  Also, if the prover tries to cheat by choosing inconsistent halves, the random challenge $r$ ensures that the probability of passing the folded relation is negligible.
\end{intuitionbox}
