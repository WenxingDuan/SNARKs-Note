\section{Chinese Remainder Theorem and the Structure of \texorpdfstring{$\mathbb{Z}_N^*$}{the unit group modulo N}}
...
\begin{remark}[Relation to Shamir secret sharing]
CRT reconstruction and Shamir reconstruction both express a global value as a linear combination of local coordinates. The analogy stops there. CRT coordinates are residues modulo pairwise coprime moduli, and the coefficients are orthogonal idempotents. Shamir shares are evaluations of one low-degree polynomial over a field, and the coefficients are Lagrange interpolation weights. In particular, Shamir's threshold privacy follows from the undetermined coefficients of a random polynomial; it does not follow merely from CRT.
\end{remark}

\subsection{Squaring in \texorpdfstring{$\mathbb{Z}_N^*$}{the unit group modulo N} via CRT}
...
\subsection{Quadratic Residues in \texorpdfstring{$\mathbb{Z}_N^*$}{the unit group modulo N}}
...
Knowing the factorization $N=pq$ makes residuosity testing and square-root extraction efficient: test each CRT component with the corresponding prime-field criterion, extract roots modulo $p$ and $q$, and recombine them. Conversely, an algorithm that returns square roots of random quadratic residues modulo a Blum integer yields a probabilistic factoring algorithm: for random $x$, obtain a root $y$ of $x^2$; whenever $y\not\equiv\pm x\pmod N$, the value $\gcd(x-y,N)$ is a nontrivial factor. This is a reduction for square-root extraction, not a blanket claim that every formulation of the quadratic-residuosity decision problem is equivalent to factoring.
